...
internal sealed class ModalTests : BUnitTestContext
{
    ...
    public void TestActive()
    {
        ...
        Assert.That(nonActiveModal.FindAll(".modal.active"), Is.Empty);

        var activeModal = RenderComponent<Modal>(builder => builder.Add(c => c.Active, true));
        Assert.That(activeModal.FindAll(".modal.active"), Is.Not.Empty);
    }
    ...
    {
        var modal = RenderComponent<Modal>(builder => builder.AddChildContent("fake-content"));
        modal.MarkupMatches("""
            <div class="modal">
